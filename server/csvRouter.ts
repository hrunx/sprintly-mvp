      ...
      try {
        ...
        for (const record of records) {
          const companyData = {
            ...
            teamSize: record.teamSize || record['Team Size'] ? parseInt(record.teamSize || record['Team Size']) : null,
            businessModel: record.businessModel || record['Business Model'] || null,
            fundingRound: record.fundingRound || record['Funding Round'] || null,
            seeking: record.seeking || record.Seeking ? parseInt(record.seeking || record.Seeking) : null,
            alreadyRaised: record.alreadyRaised || record['Already Raised'] ? parseInt(record.alreadyRaised || record['Already Raised']) : null,
            valuation: record.valuation || record.Valuation ? parseInt(record.valuation || record.Valuation) : null,
            annualRevenue: record.annualRevenue || record['Annual Revenue'] ? parseInt(record.annualRevenue || record['Annual Revenue']) : null,
            revenueGrowth: record.revenueGrowth || record['Revenue Growth'] ? parseInt(record.revenueGrowth || record['Revenue Growth']) : null,
            customers: record.customers || record.Customers ? parseInt(record.customers || record.Customers) : null,
            mrr: record.mrr || record.MRR ? parseInt(record.mrr || record.MRR) : null,
      ...
      try {
        ...
        for (const record of records) {
          const investorData = {
            ...
            checkSizeMin: record.checkSizeMin || record['Check Size Min'] ? parseInt(record.checkSizeMin || record['Check Size Min']) : null,
            checkSizeMax: record.checkSizeMax || record['Check Size Max'] ? parseInt(record.checkSizeMax || record['Check Size Max']) : null,
